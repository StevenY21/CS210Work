...
writeResults(long long result) {
  ...
  // writes SUM_POSITIVE to stdout
  if (write(1, &SUM_POSITIVE, sizeof(SUM_NEGATIVE)) != sizeof(SUM_NEGATIVE)) {
    fprintf(stderr, "ERROR: write SUM_POSITIVE failed\n");
  }
  // writes SUM_NEGATIVE to stdout
  if (write(1, &SUM_NEGATIVE, sizeof(SUM_NEGATIVE)) != sizeof(SUM_NEGATIVE)) {
    fprintf(stderr, "ERROR: write SUM_NEGATIVE failed\n");
  }

  // writes all the data memory between CALC_DATA_BEGIN and CALC_DATA_END to stdout
