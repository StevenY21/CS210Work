...
clistsum_read(void)
{
  ...
  while (1) {
    ...
    if (head == NULL) { // if first node does not exist yet, make the first node
      head = malloc(sizeof(struct Node));
      head->v = val; head->next = NULL;
    } else { // if first node exists, add onto the list
      tmp = head; 
      while (tmp->next != NULL) { //traverses the list until it reaches end of list
	tmp = tmp->next;
      }
      tmp->next = malloc(sizeof(struct Node));
      tmp->next->v = val; tmp->next->next = NULL;
    }
  }
  while (getchar()!='\n');
  return head;
