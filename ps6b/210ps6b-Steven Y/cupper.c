...
cupper(long long cres, char *cptr)
{
  ...
  for (i=0; cptr[i] != 0; i++) {
    ...
    if ((c >= 'a') && (c <= 'z')) {
      ...
      c = c - ('a' - 'A'); // 'a' - 'A' gets the decimal value 32, brings lowercase value to uppercase value
      cptr[i] = c; //makes cptr[i] point to the address that holds the uppercase version of itself
    }
  }
  return cres + i;
}
